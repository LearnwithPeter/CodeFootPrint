...
import { Mail, MessageSquare, CheckCircle, ExternalLink, AtSign, Clock, Users, Star } from 'lucide-react'

// Genuine, small-scale waitlist members
const WAITLIST_MEMBERS = [
  { name: 'Rahul Gupta', role: 'Backend Dev @ Swiggy', time: '2 days ago', avatar: 'RG', color: '#00D4FF' },
  { name: 'Tanmay Joshi', role: 'CS Student, IIT Bombay', time: '3 days ago', avatar: 'TJ', color: '#00FF88' },
  { name: 'Sneha Kapoor', role: 'Team Lead @ Razorpay', time: '5 days ago', avatar: 'SK', color: '#FFB800' },
  { name: 'Aditya Verma', role: 'Freelance Dev', time: '1 week ago', avatar: 'AV', color: '#A78BFA' },
  { name: 'Meera Iyer', role: 'Open Source Contributor', time: '1 week ago', avatar: 'MI', color: '#FB923C' },
]
    ...
    <div className="min-h-screen pt-24 pb-20">
      ...
      <section className="py-16 px-6 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto relative text-center">
          <span className="font-mono text-xs text-[#00D4FF] tracking-widest uppercase">Contact & Waitlist</span>
          ...
          <p className="font-body text-lg text-[#718096] max-w-xl mx-auto">
            We're adding private repo support and team dashboards. Join the waitlist or reach out directly.
          </p>
        </div>
      </section>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-[#0D1117] border border-[#1E2A3A] rounded-xl p-6 relative overflow-hidden">
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-mono text-xs text-[#00FF88]">Waitlist open</span>
            </div>
            <h2 className="font-display font-700 text-2xl text-white mb-1">Early access</h2>
            <p className="font-body text-sm text-[#4A5568] mb-6">
              Private repo analysis, CSV exports, team dashboards — coming soon. Be first.
            </p>
            ...
            {!joinedWaitlist ? (
              <div className="flex gap-2">
                ...
                />
                <button onClick={handleWaitlist} className="btn-primary px-5 py-2.5 rounded-lg font-display text-sm text-black font-600 whitespace-nowrap">
                  <span>Join →</span>
                </button>
              </div>
            ) : (
              <div className="flex items-center gap-3 py-3">
                ...
                <span className="font-body text-sm text-[#00FF88]">You're on the list. We'll email you when ready.</span>
              </div>
            )}
            ...
            <div className="mt-6 pt-5 border-t border-[#1E2A3A]">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-body text-xs text-[#4A5568]">Recently joined</span>
                </div>
                <span className="font-mono text-xs text-[#00D4FF]">17 members</span>
              </div>
                ...
                {WAITLIST_MEMBERS.map(member => (
                  <div key={member.name} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-body text-sm text-white truncate">{member.name}</div>
                      <div className="font-body text-xs text-[#4A5568] truncate">{member.role}</div>
                    </div>
              ...
              <div className="mt-3 text-center">
                <span className="font-mono text-xs text-[#2D3748]">+ 12 others joined this week</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-body text-sm text-[#4A5568]">What early users say</span>
            </div>
            {REVIEWS.map(review => (
              <div key={review.name} className="card-hover bg-[#0D1117] border border-[#1E2A3A] rounded-xl p-5">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <div className="font-body text-sm font-500 text-white">{review.name}</div>
                    <div className="font-body text-xs text-[#4A5568]">{review.role}</div>
                  </div>
                  <div className="ml-auto flex gap-0.5">
                    {Array.from({ length: review.stars }).map((_, i) => (
                      <Star key={i} size={11} className="text-[#FFB800] fill-[#FFB800]" />
                    ))}
                  </div>
                </div>
                <p className="font-body text-sm text-[#718096] leading-relaxed italic">"{review.text}"</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <div className="bg-[#0D1117] border border-[#1E2A3A] rounded-xl p-6 sticky top-28">
            ...
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="font-display font-600 text-white text-xl">Send a message</h2>
            </div>

            {!submitted ? (
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block font-body text-xs text-[#4A5568] mb-1.5">Your name</label>
                  <input
                    type="text"
                    value={form.name}
                  ...
                  />
                  {errors.name && <p className="font-mono text-xs text-[#FF3B5C] mt-1">{errors.name}</p>}
                </div>

                <div>
                  <label className="block font-body text-xs text-[#4A5568] mb-1.5">Email</label>
                  <input
                    type="email"
                    value={form.email}
                  ...
                  />
                  {errors.email && <p className="font-mono text-xs text-[#FF3B5C] mt-1">{errors.email}</p>}
                </div>

                <div>
                  <label className="block font-body text-xs text-[#4A5568] mb-1.5">Message</label>
                  <textarea
                    rows={5}
                    value={form.message}
                  ...
                  />
                  {errors.message && <p className="font-mono text-xs text-[#FF3B5C] mt-1">{errors.message}</p>}
                </div>

                <button type="submit" className="btn-primary w-full py-3 rounded-lg font-display font-600 text-black text-sm">
              ...
              <div className="py-12 text-center">
                ...
                <h3 className="font-display font-600 text-white text-xl mb-2">Message sent!</h3>
                <p className="font-body text-sm text-[#4A5568]">We'll get back to you at {form.email}</p>
                <button
            ...
            <div className="mt-6 pt-5 border-t border-[#1E2A3A] flex items-center justify-between">
              <span className="font-body text-xs text-[#4A5568]">Or reach us on</span>
              <div className="flex items-center gap-3">
                ...
                <a href="mailto:hello@codefootprint.dev"
                  className="p-2 border border-[#1E2A3A] rounded-lg text-[#4A5568] hover:text-[#00D4FF] hover:border-[#00D4FF44] transition-all">
                  <Mail size={15} />
                </a>
              </div>
